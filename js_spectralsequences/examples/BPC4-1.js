...
let b;
const sequentialTuples = (a) => [
  a.map((a) => a[0]),
  ...((b = a.find((a) => a[1])) ? sequentialTuples(a, b.shift()) : []),
];
...
class sliceMonomial {
  ...
  toString() {
    return subnomialString(
      ...
      monomialString(
        [
          "\\overline{\\mathfrak{d}}_3",
          "\\overline{\\mathfrak{d}}_1",
          "\\overline{s}_1",
        ],
        [this.d3, this.d1, this.s1],
      ),
    );
  }
...
function getSlice(d1pow, s1pow) {
  ...
  let tuples;
  if (s1pow === 0) {
    tuples = sequentialTuples([
      [0],
      range(0, d1pow),
      range(d1pow % 2, d1pow, 2),
    ]);
  } else {
    tuples = sequentialTuples([
      range(s1pow % 2, s1pow, 2),
      range(0, d1pow),
      [0],
    ]);
  }
...
const slices = BPC4.addSliceClasses(
  {
...
slices.addDifferential(
  3,
  [0, 1],
  (k, stem, filtration) => (stem - filtration) % 8 === 4,
);
...
for (let d1 = 0; d1 < d1max; d1 += 2) {
  for (let s = 0; s <= 1; s++) {
    BPC4.addStructline(
      slices.get([d1, s]).get(4 * d1 + s),
      slices.get([d1, s + 1]).get(4 * d1 + s + 1),
    ).setMinPage(5);
  }
}
...
function addDifferential(
  ...
  slices.addDifferentialLeibniz(
    ...
    [
      [0, Math.floor(d1max / (2 * i))],
      [0, Math.floor(d1max / (4 * i))],
    ],
  );
}
...
window.saveSseq = function saveTruncationSseq(sseq) {
  ...
  for (let c of sseq
    ...
    .filter(
      (c) =>
        c.page_list[c.page_list.length - 1] > 3 && c.x < max_x && c.y < max_y,
    )) {
    let o = {};
    o.name = c.name;
