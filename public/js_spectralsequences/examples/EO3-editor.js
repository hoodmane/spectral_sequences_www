...
sidebar.footer.addButton("Save", () =>
  sseq.download(sseq.name ? sseq.name : "sseq.json"),
);

generalPanel = new Panel.Panel(sidebar.main_div, display);
generalPanel.newGroup();
generalPanel.addButton("New sseq", () => new_sseq_form.open(), {
  shortcuts: ["n"],
}); // Use arrow function because new_sseq_form is not yet defined
generalPanel.addButton("Open sseq", () => open_sseq_form.open(), {
  shortcuts: ["o"],
});
generalPanel.addButton(
  "Add class",
  () => {
...
differentialPanel.addButton(
  "Add extension",
  () => (display.state = STATE_ADD_EXTENSION),
  { shortcuts: ["e"] },
);
...
AHSS.addCell = function addAHSSCell(sseq, cell_dim) {
  ...
  for (let v = vmin; v < vmax; v++) {
    ...
    let o = AHSS.setClassName(sseq.addClass(0, 0), [0, 0, 0, 3 * v], cell_dim)
      .setNode(squareNode)
      .setColor(colorList[i]);
    AHSS.setClassName(sseq.addClass(24, 0), [0, 0, 0, 3 * v + 1], cell_dim)
      .setNode(openSquareNode)
      .setColor(colorList[i]);
    AHSS.setClassName(sseq.addClass(48, 0), [0, 0, 0, 3 * v + 2], cell_dim)
      .setNode(openSquareNode)
      .setColor(colorList[i]);
    let b = AHSS.setClassName(sseq.addClass(10, 2), [0, 1, 0, 3 * v], cell_dim);
    let b2 = AHSS.setClassName(
      sseq.addClass(20, 4),
      [0, 2, 0, 3 * v],
      cell_dim,
    );
    let b3 = AHSS.setClassName(
      sseq.addClass(30, 6),
      [0, 3, 0, 3 * v],
      cell_dim,
    );
    let b4 = AHSS.setClassName(
      sseq.addClass(40, 8),
      [0, 4, 0, 3 * v],
      cell_dim,
    );
    let a = AHSS.setClassName(sseq.addClass(3, 1), [1, 0, 0, 3 * v], cell_dim);
    let ab = AHSS.setClassName(
      sseq.addClass(13, 3),
      [1, 1, 0, 3 * v],
      cell_dim,
    );
    let x = AHSS.setClassName(sseq.addClass(27, 1), [0, 0, 1, 3 * v], cell_dim);
    let bx = AHSS.setClassName(
      sseq.addClass(37, 3),
      [0, 1, 1, 3 * v],
      cell_dim,
    );
...
function addDifferential(type, s, t, length) {
  ...
  sseq.addMutationsToUndoStack({
    command: "addDifferential",
    arguments: [s.exponents, t.exponents, length],
  });
}
...
AHSS.addDifferential = function addDifferentialAHSS(sc, tc, length) {
  ...
  for (let v = vmin; v < vmax; v++) {
    ...
    if (
      !sseq.exponents_to_classes.has(scexp) ||
      !sseq.exponents_to_classes.has(tcexp)
    ) {
      continue;
    }
    let e = sseq.addDifferential(
      sseq.exponents_to_classes.get(scexp),
      sseq.exponents_to_classes.get(tcexp),
      length,
    );
  }
};
...
function addExtension(type, s, t, flags) {
  ...
  sseq.addMutationsToUndoStack({
    ...
    arguments: [s.exponents, t.exponents, flags],
  });
}
commands.addExtension = addExtension;

AHSS.addExtensionQueries = function addExtensionQueries() {
  return {};
};
