...
IO.loadFromServer(getJSONFilename("BPC4-2-E13"))
  .then(function (json) {
...
function addDifferentialsLogic() {
  for (let v = 16; v < max_diagonal; v += 32) {
    for (let i = 0; i < max_diagonal; i++) {
      let d = sseq.addDifferential(
        classes.induced.get([v + i, i]),
        classes.induced.get([v + i - 1, i + 15]),
        15,
      );
      // console.log("hi");
      if (d.isDummy()) {
  ...
  function addDifferential(
    ...
    source_class_dict = classes.truncation,
    target_class_dict = classes.truncation,
  ) {
    ...
    for (let exponent_vector of product(
      ...
      let source_degree = vectorSum(
        cur_source,
        vectorLinearCombination(offset_vectors, exponent_vector),
      );
